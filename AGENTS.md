...
- `GET /accounts/leaderboard` (#56, same `ACCOUNTS_READ_TOKEN` bearer, same server-to-server-only rule) is the **one cross-player aggregate** on this surface: `{ players: [{ playerId, gamesPlayed, wins }] }`, one row per player with at least one completed game. It is `playerTotals` with `WHERE s.player_id = $1` lifted into a `GROUP BY` — same DISTINCT ON per `(player, game)`, same `campaign_id IS NULL` completed-game filter, same `won` win predicate — so a leaderboard row always equals what that player's own `/stats` reports. XP lives api-side (tiered weights telemetry does not know), so rows cannot be pre-sorted by it: they come back `gamesPlayed` desc and unbrewed-api sorts. `limit` is an optional safety cap; the default is unlimited because the user base is small. **Treat the payload shape as fixed** — unbrewed-api#22 and unbrewed-p2p#590 are written against it.
