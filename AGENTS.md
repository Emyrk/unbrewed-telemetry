...
- `/v1/admin/campaigns` and related admin routes create and inspect deterministic campaigns. Campaigns may use `gameCount` for large repeated runs or explicit per-game overrides. Shared specs use checkbox-style `maps` pools and per-seat `decks`/`pilots` pools with stable hero IDs; the service deterministically resolves pools into exact job specs. `swapStartingPlayer` is first-class, and omitted base seeds default to Unix nanoseconds returned as decimal strings. Active/paused campaigns are arranged into ordered priority tiers via `PUT /v1/admin/campaign/schedule`: lower tiers must finish before later tiers become claimable, while campaigns side by side in one tier are claimed round-robin. Completed/cancelled campaigns are archival and do not participate in scheduling. `GET /v1/admin/campaign/submissions?id=...` returns that campaign's completed-game submission totals grouped by telemetry source and bearer credential label.
- `GET /accounts/players/{playerId}/games` and `GET /accounts/players/{playerId}/stats` (**server-to-server only**, `Authorization: Bearer $ACCOUNTS_READ_TOKEN`) serve a signed-in player their own game history and lifetime stats to the unbrewed accounts service (`unbrewed-api`), which proxies them — a browser must never reach these directly. Keyed on `game_seats.player_id`, the pseudonymous account uuid the engine stamps on a signed-in seat. Both are read-only, exclude sim/campaign games (`games.campaign_id IS NOT NULL`), and return empty results (not 404) for an unknown player id. `games` paginates newest-first with an **opaque** cursor (`nextBefore` → `before`); `limit` defaults to 20 and caps at 50. `ACCOUNTS_READ_TOKEN` is a deliberate *third* credential, distinct from the `ubk_` bearer scheme and legacy HMAC: read-only blast radius, independently rotatable, and unset means 503 rather than fail-open. Reads existing `games`/`game_seats`; no new tables (migration `013` adds a partial index on `game_seats.player_id`). Beyond the lifetime totals and per-hero rows, `stats` carries (#54) `avgDurationSeconds`/`avgTurns`, win `streaks` (`current`/`best`, a draw breaks a run), `recentForm` (last 10 results newest first), `byOpponentHero`, `byMap`, `byOpponentKind` (human vs per-tier bots), and a `firstPlayer` split. Bot tiers are decoded from `game_seats.pilot` (#58) — the live engine path stamped no `bot_difficulty` at all, so keying on it collapsed every bot game into one `unknown` row; the label→tier map is `src/db/bot-tier.ts`, which also generates the SQL `CASE` the aggregates group on, and a stamped `bot_difficulty` still takes precedence. Migration `014_backfill_bot_difficulty.sql` (#60) fills that column in for the five **current-era serving presets** only (`bot:easy`, `bot:medium`, `bot:mc(16,10000ms)`, `bot:mc(64,10000ms)`, `bot:ismcts(512,10000ms)`, matched on a whitespace/case-normalized label) so unbrewed-api's difficulty-gated badges can grant retroactively; legacy budget labels such as `bot:mc(64, 400ms)` stay NULL on purpose and keep reading their tier through `bot-tier.ts`. It only touches `bot_difficulty IS NULL`, so it is a no-op on a re-run and never overwrites what the engine sends after unbrewed-engine#366. Every split row (human, each bot tier, both `firstPlayer` sides) carries `games`/`wins`/`draws` so a client can derive per-slice losses. Each `byHero[]` row also carries `byOpponent` (#63) — the cross of the two, `{ human, easy, medium, hard, expert }` each `{ games, wins }`, five fixed keys always present — which is what the cosmetics point system (unbrewed-p2p#610) prices per-hero rewards off; it reuses `byOpponentKind`'s classification exactly, so a bot side decoding to `unknown` (and a game with no opposing seat) sits in no bucket and the buckets can sum to less than the row's `games`. Those `byOpponent` counts are **what may be paid for, not the raw record** (#66): a concession (`end_condition` in `forfeit`/`timeout`/`disconnect`, `CONCESSION_END_CONDITIONS`) gives the conceding seat neither `games` nor `wins` and the winning seat `games` without `wins`, and a `human`-bucket win under `MIN_HUMAN_WIN_TURNS` (5) turns gives `games` without `wins` — bot buckets exempt (a fast expert kill is real; a bot cannot collude), NULL `turns` passes, draws untouched. Both are per-game predicates, so they live in `playerByHeroOpponent` and not in unbrewed-api, and because points are recomputed on read they backfill themselves. `byHero[].games`/`wins`, `byOpponentKind` and the records stay unfiltered — same scoping as `minSeconds`. `?minSeconds=<n>` on `stats` floors `games.duration_seconds` over **that block alone** — every other number counts the full history — with `0` (the default, and what a blank/negative/unparseable value clamps to) meaning no filter and any positive floor also excluding games that reported no duration at all. All are computed in SQL — eight grouped queries over two shared CTEs, which is affordable because unbrewed-api caches this per user for 60s. Matchup and opponent-kind rows count *opposing* seats only (teammates excluded, so a 2v2 yields two opponent-hero rows), a game with any bot opponent counts as a bot game, and games with no reported `first_player_team` sit out the first/second split. **Treat the payload shape as fixed** — the client stats page is written against it.
- `GET /accounts/leaderboard` (#56, same `ACCOUNTS_READ_TOKEN` bearer, same server-to-server-only rule) is the **one cross-player aggregate** on this surface: `{ players: [{ playerId, gamesPlayed, wins, byOpponentKind }] }`, one row per player with at least one completed game. It is `playerTotals` + `playerByOpponentKind` with `WHERE s.player_id = $1` lifted into a `GROUP BY` — same DISTINCT ON per `(player, game)`, same `campaign_id IS NULL` completed-game filter, same `won` win predicate, same any-bot-opponent-means-bot-game classification over opposing seats only, same pilot-label tier decoding — so a leaderboard row always equals what that player's own `/stats` reports. `byOpponentKind` is there because unbrewed-api's XP is *tiered by opponent* (a human win is worth several times an easy-bot win); without it the api has to weight every game as human and over-states bot-heavy players. XP lives api-side (tiered weights telemetry does not know), so rows cannot be pre-sorted by it: they come back `gamesPlayed` desc and unbrewed-api sorts. `limit` is an optional safety cap; the default is unlimited because the user base is small. **Treat the payload shape as fixed** — unbrewed-api#22 and unbrewed-p2p#590 are written against it.
