...
- `GET /v1/stats/pilot-comparison?pilotA=&pilotB=&opponentPilot=&hero=&opponent=` compares two exact pilots in 1v1 while holding the opposing pilot constant. Without `hero`, rows summarize active heroes; with `hero`, rows list that hero against each opposing hero. Powers the Pilot Comparisons dashboard tab.
